# Contributing to Mosofin

Thank you for helping Mosofin make engineering diagrams more trustworthy and useful. Mosofin is Agent-first: people describe the system they want to explain, while the Skill, typed JSON contract, renderers, validators, and delivery receipts make the result reproducible.
...
Mosofin's public behavior is larger than a renderer function. Treat these as contracts:
...
The renderer package lives in `mosofin/` and supports Node.js 18 and later. CI covers Node.js 18, 20, 22, and 24.
...
Test public behavior through a supported seam whenever possible: `mosofin render`, `validate`, `deliver`, `visual-check`, or the final SVG/HTML. Private helper tests are useful for edge cases, but they do not replace a CLI or artifact-level regression.
...
```bash
cd mosofin
MOSOFIN_CHROME="/path/to/chrome" node --test test/desktop-reader-browser.test.mjs
```
...
Preserve non-zero exit behavior and machine-readable receipts. A successful `validate` does not prove atomic delivery, and a successful `deliver` does not prove perceptual quality. Follow [the delivery contract](mosofin/references/delivery-contract.md) and test the failure stage you changed.
...
scripts/build-zip.sh /tmp/mosofin-contrib.zip
```

The runtime follows the Node range in `mosofin/package.json`, but canonical
`mosofin.zip` container bytes are built only with Node 22. The builder rejects
...
1. The exact Mosofin version or commit, installation method, command, and environment.
...
Showcase cases should be reproducible proof, not promotional screenshots. Submit the original prompt, agent/client, exact model, Mosofin version, redacted typed JSON, artifact, validation receipt, and truthful visual-review status through `.github/ISSUE_TEMPLATE/showcase.yml`.
