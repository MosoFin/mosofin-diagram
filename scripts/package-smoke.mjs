...
const defaultPackageRoot = process.env.RUNNER_TEMP
  ? path.join(process.env.RUNNER_TEMP, 'mosofin-package', 'mosofin')
  : path.join(repoRoot, 'mosofin');
const skillRoot = path.resolve(process.argv[2] || defaultPackageRoot);
const cli = path.join(skillRoot, 'bin', 'mosofin.mjs');
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-package-smoke-'));
...
function run(args, options = {}) {
  ...
  if (result.status !== 0) {
    throw new Error([
      `mosofin ${args.join(' ')} failed with ${result.status}`,
      result.stdout,
      result.stderr,
    ].filter(Boolean).join('\n'));
  }
...
function runExpectFailure(args, options = {}) {
  ...
  if (result.status === 0) throw new Error(`mosofin ${args.join(' ')} unexpectedly passed`);
  return result.stdout;
}
...
try {
  ...
  const visualSkipped = JSON.parse(runExpectFailure([
    ...
    env: { ...process.env, MOSOFIN_CHROME: path.join(scratch, 'missing-chrome') },
  }));
