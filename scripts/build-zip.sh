#!/usr/bin/env bash
# Build the distributable skill archive from the mosofin/ folder.
# Usage: scripts/build-zip.sh [output.zip]
set -euo pipefail

repo_root="$(cd "$(dirname "$0")/.." && pwd)"
out="${1:-$repo_root/mosofin.zip}"
if [[ "$out" != /* ]]; then
  out="$(pwd)/$out"
fi

# Runtime consumers support every Node version declared by mosofin/package.json,
...
if [[ "$node_major" != "$canonical_node_major" ]]; then
  echo "canonical mosofin.zip builds require Node $canonical_node_major (current: $node_version)" >&2
  exit 1
fi
...
if [[ ! -f "$repo_root/mosofin/renderers/shared/generated-validators.mjs" ]]; then
  echo 'generated validators are missing — run npm run generate:validators in mosofin/' >&2
  exit 1
fi
...
done < <(git -C "$repo_root" ls-files --stage -z -- mosofin)
node -e "
  const fs = require('fs');
  const p = '$stage/mosofin/package.json';
  const pkg = JSON.parse(fs.readFileSync(p, 'utf8'));
  delete pkg.scripts;
  delete pkg.devDependencies;
  fs.writeFileSync(p, JSON.stringify(pkg, null, 2) + '\n');
"
rm -f "$stage/mosofin/package-lock.json"

node "$repo_root/scripts/write-deterministic-zip.mjs" "$stage/mosofin" "$out"
