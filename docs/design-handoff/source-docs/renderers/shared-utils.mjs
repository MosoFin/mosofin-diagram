...
const SVG_SLOT_RE = /      <!-- MOSOFIN:SVG_SLOT_START -->[\s\S]*?      <!-- MOSOFIN:SVG_SLOT_END -->/;
const CARDS_SLOT_RE = /    <!-- MOSOFIN:CARDS_SLOT_START -->[\s\S]*?    <!-- MOSOFIN:CARDS_SLOT_END -->/;
...
const TEMPLATE_PLACEHOLDERS = [
  '<html lang="en" data-theme="dark" data-preset="[VISUAL PRESET]" data-node-style="[NODE STYLE]">',
...
export function applyTemplate(template, {
  ...
  if (!CARDS_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing MOSOFIN:CARDS_SLOT sentinel');
  }
  ...
  const renderedSubtitle = typeof subtitle === 'string' && subtitle.trim()
    ...
    : '';
  const i18nData = `    <script id="mosofin-i18n-data" type="application/json">${i18nJson}</script>`;
