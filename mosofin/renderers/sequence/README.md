...
Render `diagram_type: "sequence"` JSON files into the standard Mosofin HTML
template.

```bash
node mosofin/renderers/sequence/render-sequence.mjs input.sequence.json output.html
```

The renderer validates input against `mosofin/schemas/sequence.schema.json`
...
`mosofin/examples/cache-miss-request.sequence.json`.

The schema lives at:

```text
mosofin/schemas/sequence.schema.json
```
