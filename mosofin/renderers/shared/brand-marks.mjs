...
const RESOLVED_MARK = Symbol('mosofin.brandMark');
...
function captureTimeoutMilliseconds() {
  const configured = Number(process.env.MOSOFIN_BRAND_CAPTURE_TIMEOUT_MS);
...
export async function prepareDiagramBrandMarks(diagramType, diagram) {
  ...
  await mapConcurrent(nodes, MAX_CAPTURE_CONCURRENCY, async (node, index) => {
    ...
    if (url) {
      unknown.push(`/${collection}/${index}/brand ${JSON.stringify(node.brand)} is an unpinned URL; capture it first with \`mosofin brands capture ${url.href} --json\``);
      return;
    }
  ...
  if (unknown.length) {
    throwDiagnosticError(`Brand mark validation failed:\n- ${unknown.join('\n- ')}`, unknown.map((message) => ({
      ...
      supportedFixes: message.includes('is an unpinned URL')
        ? ['run `mosofin brands capture <url> --json` and author the returned digest-pinned brand object']
        : ['choose an ID from `mosofin brands`', 'run `mosofin brands capture <url> --json` for an unknown official site'],
    })));
  }
}
