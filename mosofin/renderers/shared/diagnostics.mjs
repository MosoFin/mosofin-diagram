...
const DIAGNOSTIC_MODE = process.env.MOSOFIN_DIAGNOSTIC_FORMAT === 'json';
const recorded = [];
const recordedMessages = new Set();
const boundaryKey = Symbol.for('mosofin.renderer-diagnostic-boundary');
...
function normalizedDiagnostic(diagnostic) {
  const message = String(diagnostic?.message || 'Mosofin could not classify this failure.').trim();
  return {
...
export function throwDiagnosticError(message, diagnostics) {
  ...
  error.mosofinDiagnostics = (diagnostics || []).map(normalizedDiagnostic);
  throw error;
}
...
function rendererFailure(error) {
  const attached = Array.isArray(error?.mosofinDiagnostics)
    ? error.mosofinDiagnostics.map(normalizedDiagnostic)
    : [];
