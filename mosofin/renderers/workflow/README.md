...
Render `diagram_type: "workflow"` JSON files into the standard Mosofin HTML
template.

```bash
node mosofin/renderers/workflow/render-workflow.mjs input.workflow.json output.html
```

The renderer validates input against `mosofin/schemas/workflow.schema.json`
...
```bash
node mosofin/scripts/check-render-output.mjs output.html
```
...
`mosofin/examples/agent-tool-call.workflow.json`.

The schema lives at:

```text
mosofin/schemas/workflow.schema.json
```
