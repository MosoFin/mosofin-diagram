...
Render `diagram_type: "lifecycle"` JSON files into the standard Mosofin HTML
template.

```bash
node mosofin/renderers/lifecycle/render-lifecycle.mjs input.lifecycle.json output.html
```

The renderer validates input against `mosofin/schemas/lifecycle.schema.json`
...
`mosofin/examples/agent-run.lifecycle.json`.

The schema lives at:

```text
mosofin/schemas/lifecycle.schema.json
```
