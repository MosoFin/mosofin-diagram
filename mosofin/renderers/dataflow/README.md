...
Render `diagram_type: "dataflow"` JSON files into the standard Mosofin HTML
template.

```bash
node mosofin/renderers/dataflow/render-dataflow.mjs input.dataflow.json output.html
```

The renderer validates input against `mosofin/schemas/dataflow.schema.json`
...
`mosofin/examples/product-analytics.dataflow.json`.

The schema lives at:

```text
mosofin/schemas/dataflow.schema.json
```
...
visual-only because Mosofin has no compiled edge-kind facts in this slice. A
