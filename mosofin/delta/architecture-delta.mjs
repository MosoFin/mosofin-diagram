...
export function compareArchitecture(base, head, evidence = {}) {
  ...
  if (!shared.length) {
    fail('delta/no-shared-component-id', 'The snapshots share no component id, so Mosofin cannot prove that they describe the same system.', {
...
<script id="mosofin-compare-receipt" type="application/json">${safeJson(receipt)}</script>
<script>(()=>{
  ...
  const receiptNode = document.querySelector('#mosofin-compare-receipt');
  ...
  function validateReview() {
    try {
      if (document.querySelectorAll('#mosofin-compare-receipt').length !== 1 || !receiptNode) return false;
  ...
  function recordExport(format, blob, dimensions) {
    document.documentElement.dataset.mosofinDeltaExport = JSON.stringify({
      ok: true,
      format,
      bytes: blob.size,
      mime: blob.type,
  ...
  window.Mosofin = window.Mosofin || {};
  window.Mosofin.deltaExport = { canonicalSvg: canonicalDeltaSvg, shareCard, exportSvg: exportCanonicalSvg, downloadShareCard };
  window.Mosofin.exportMenu = {
    shareCard,
    run(format) {
...
export function validateArchitectureDeltaHtml(html, receipt) {
  ...
  if ((html.match(/id="mosofin-compare-receipt"/g) || []).length !== 1) failures.push('expected exactly one embedded compare receipt');
  ...
  if (!html.includes('id="export-svg"') || !html.includes('id="share-card"')
    || !html.includes('window.Mosofin.deltaExport = { canonicalSvg: canonicalDeltaSvg, shareCard')) {
