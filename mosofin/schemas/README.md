...
| `ledger.schema.json` | `diagram_type: "ledger"` | `stages`, `accounts`, `entities`, `flows`, `ledger` (period, proof, events, unmapped, tieouts) |
...
explicit authored viewBox without overlap, Mosofin omits the complete legend
...
by `mosofin brands --json`, or a digest-pinned `{ "url", "sha256" }` object
returned by `mosofin brands capture <url> --json`. Known IDs and known-brand
...
strictly Mosofin judges composition. `engineering_profile` is an optional
