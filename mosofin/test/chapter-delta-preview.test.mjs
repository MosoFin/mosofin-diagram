...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-chapter-delta-preview-'));
...
test('pointer and keyboard inspect while touch and native activation still commit directly', () => {
  ...
  assert.doesNotMatch(previewRuntime, /Mosofin\.view\.|updateUrl\(|Mosofin\.focus\.|renderStoryTrail\(/);
});
...
test('latest intent, stronger owners, playback, and lifecycle cleanup remain bounded', () => {
  ...
  assert.match(html, /Mosofin\.motionGovernor\.claim\('chapter-preview'/);
  assert.match(html, /Mosofin\.motionGovernor\.release\(token\)/);
