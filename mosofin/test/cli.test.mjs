...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-cli-'));
...
fs.writeFileSync(process.env.MOSOFIN_TEST_OPEN_LOG, JSON.stringify({
...
test('cli: help lists commands and diagram types', () => {
  ...
  assert.match(result.stdout, /mosofin render <type>/);
  assert.match(result.stdout, /mosofin compare architecture <base\.json> <head\.json>/);
  assert.match(result.stdout, /mosofin deliver <type>/);
  assert.match(result.stdout, /mosofin preview <type>/);
  assert.match(result.stdout, /mosofin visual-check <output\.html>/);
...
test('cli: doctor identifies an incomplete installation', () => {
  ...
  fs.copyFileSync(cli, path.join(incompleteBin, 'mosofin.mjs'));

  const result = spawnSync(process.execPath, [path.join(incompleteBin, 'mosofin.mjs'), 'doctor'], {
    cwd: incompleteRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /Mosofin is not ready: \d+ required files? missing\./);
});
...
test('cli: doctor rejects a corrupt standalone validator', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(corruptRoot, 'bin/mosofin.mjs'), 'doctor'], {
    cwd: corruptRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /Mosofin is not ready: 1 runtime check failed\./);
});
...
test('cli: examples renders from an installed skill', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(installedRoot, 'bin/mosofin.mjs'), 'examples'], {
    cwd: installedRoot,
    encoding: 'utf8',
  });
...
test('cli: guide works from an installed skill without node_modules', () => {
  ...
  copyInstalledSkill(installedRoot);
  const installedCli = path.join(installedRoot, 'bin/mosofin.mjs');
...
test('cli: demo creates a ready-to-open diagram in a chosen directory', () => {
  ...
  assert.match(result.stdout, /mosofin render architecture/);
});
...
test('cli: demo defaults to the current directory', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.equal(fs.existsSync(path.join(workingDirectory, 'mosofin-demo.html')), true);
});
...
test('cli: visual-check returns a skipped receipt with exit 2 when Chrome is unavailable', () => {
  ...
  const result = run(['visual-check', out, '--json'], {
    env: { ...process.env, MOSOFIN_CHROME: missingChrome },
  });
...
test('cli: preview runs from an installed skill without node_modules and exits cleanly', { timeout: 30000 }, async () => {
  ...
  assert.deepEqual(fs.readdirSync(path.dirname(output)).filter((name) => name.startsWith('.mosofin-preview-')), []);
});
...
test('cli: deliver preserves the previous artifact when the final check fails', () => {
  ...
  assert.deepEqual(
    fs.readdirSync(path.dirname(out)).filter((name) => name.includes('.mosofin-delivery-')),
    [],
  );
});
...
test('cli: inspect emits architecture layout json', () => {
  const input = path.resolve(skillRoot, '../examples/rag-pipeline.architecture.json');
  const result = run(['inspect', 'architecture', input]);
  assert.equal(result.status, 0, result.stderr);
  const parsed = JSON.parse(result.stdout);
  assert.equal(parsed.ok, true);
  assert.equal(parsed.diagram_type, 'architecture');
  assert.ok(['grid', 'free'].includes(parsed.layout.mode));
