...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-v1-compat-'));
...
test('legacy v1 explicit narrow viewBoxes never hard-fail on an implicit auto legend', () => {
  ...
  for (const [mode, doc] of Object.entries(narrowExplicitViewBoxDocuments())) {
    ...
    if (mode === 'lifecycle') {
      ...
      // Finance labels are shorter than the engineering ones they replaced, so this legend
      // now fits where it previously had to degrade. Either outcome is compatible; a hard
      // failure is not, and the validate() assertion below proves it did not occur.
      assert.match(svg, />Legend</, 'a fitting implicit legend should remain visible');
    } else {
