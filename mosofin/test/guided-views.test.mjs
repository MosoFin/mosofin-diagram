...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-guided-views-'));
...
for (const [mode, config] of Object.entries(CASES)) {
  test(`${mode}: guided views preserve base SVG geometry`, () => {
    ...
    assert.match(guided.html, /Mosofin\.guidedViews = \(function \(\)/);
    ...
    assert.match(guided.html, /--mosofin-scroll-x/);
