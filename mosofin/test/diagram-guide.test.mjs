...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-diagram-guide-'));
...
test('all typed renderers inherit one viewer-only Diagram Guide', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Mosofin\.guide = \(function \(\)/, mode);
    assert.match(html, /Diagram Guide — a factual command deck over existing interactions/, mode);
    assert.doesNotMatch(canonicalSvg(html), /diagram-guide|Mosofin\.guide|Explore this system/, mode);
  }
});
...
test('Diagram Guide reports compiled semantic facts and honest story availability', () => {
  ...
  assert.match(html, /return Mosofin\.guidedViews && Number\(Mosofin\.guidedViews\.count\) \|\| 0/);
...
test('Diagram Guide delegates its task rows to existing production interactions', () => {
  ...
  assert.match(html, /if \(action === 'find'\) return Mosofin\.finder\.open\(\)/);
  assert.match(html, /if \(action === 'route'\) return Mosofin\.routeProbe\.begin\(\{ focusNode: true \}\)/);
  assert.match(html, /if \(action === 'map'\) return Mosofin\.radar\.open\(\)/);
  assert.match(html, /if \(action === 'story'\) return Mosofin\.guidedViews\.play\(\)/);
  assert.match(html, /if \(action === 'present'\) return Mosofin\.presentation\.enter\(\)/);
  assert.match(html, /if \(action === 'export'\) return Mosofin\.exportMenu\.open\(\)/);
  assert.match(html, /if \(action === 'theme'\) return Mosofin\.theme\.toggle\(\)/);
  assert.match(html, /if \(action === 'reset'\) return Mosofin\.view\.reset\(\)/);
  assert.match(html, /Mosofin\.guidedViews\.pause\(\)/);
  assert.match(html, /Mosofin\.finder\.close\(\{ restoreFocus: false \}\)/);
  assert.match(html, /Mosofin\.radar\.close\(\{ restoreFocus: false \}\)/);
...
test('Diagram Guide is keyboard-first, mobile-contained, motion-safe, and embed-clean', () => {
  ...
  assert.match(html, /Mosofin\.guide\.toggle\(\)/);
