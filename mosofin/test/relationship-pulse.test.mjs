...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-relationship-pulse-'));
...
test('semantic tokens use five distinct inline SVG cues on one finite timing owner', () => {
  ...
  assert.match(html, /animation: mosofin-relationship-token-life 1\.2s linear 1 both/);
  assert.match(html, /semantic-flow-token-halo/);
  assert.match(html, /Mosofin\.flowTokens = \{/);
...
test('pulse is finite, event-owned, preset-aware, touch-safe, and motion-safe', () => {
  ...
  assert.match(html, /animation: mosofin-relationship-pulse 1\.2s linear 1 both/);
  assert.match(html, /@keyframes mosofin-relationship-token-life/);
  ...
  assert.match(html, /Mosofin\.motionGovernor && Mosofin\.motionGovernor\.isPaused\(\)/);
