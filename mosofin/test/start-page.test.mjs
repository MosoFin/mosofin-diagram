...
test('start page: checked-in HTML is reproducible from canonical scenario recipes', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-start-page-'));
...
test('start page: offers six bounded bilingual starts without ingesting source content', () => {
  ...
  assert.match(html, /npx -y skills add MosoFin\/mosofin-diagram --skill mosofin --agent codex --global --copy --yes/);
  assert.match(html, /npx -y skills add MosoFin\/mosofin-diagram --skill mosofin --agent codex --copy --yes/);
  ...
  assert.deepEqual(Object.keys(data), ['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'ledger']);
  ...
  assert.match(scriptMatch[1], /mosofin:start-funnel/);
  assert.match(scriptMatch[1], /mosofin\.start\.events\.v1/);
...
test('generated artifacts omit the promotional footer and shortcut manual', () => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-start-artifacts-'));
  try {
    for (const [type, input] of Object.entries(examples)) {
      ...
      assert.doesNotMatch(html, /Built with Mosofin/, `${type}: product signature`);
...
test('viewer gives wide screens a larger canvas without forcing a subtitle row', () => {
  ...
  assert.match(template, /max-width: var\(--mosofin-reader-width, 1440px\)/);
  assert.match(template, /Mosofin\.readerLayout = \(function \(\)/);

  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-title-hierarchy-'));
  try {
