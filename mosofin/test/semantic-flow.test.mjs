...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-semantic-flow-'));
...
test('Semantic Flow has preset identities and motion-safe density boundaries', () => {
  ...
  assert.match(html, /@keyframes mosofin-semantic-lens-flow/);
  assert.match(html, /animation: mosofin-semantic-lens-flow 1\.35s linear 1 both/);
