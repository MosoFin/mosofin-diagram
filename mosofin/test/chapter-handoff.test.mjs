...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-chapter-handoff-'));
...
test('handoff holds one truthful anchor then settles through one finite camera transaction', () => {
  ...
  assert.match(html, /Mosofin\.motionGovernor\.claim\('handoff'/);
  assert.match(html, /Mosofin\.motionGovernor\.release\(handoff\.ownerToken\)/);
...
test('latest intent, manual takeover, Still, reduced motion, and hidden pages cleanly settle', () => {
  ...
  assert.match(html, /Mosofin\.guidedViews\.cancelHandoff\(reason \|\| 'manual'\)/);
