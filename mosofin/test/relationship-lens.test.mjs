...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-relationship-lens-'));
...
test('relationship lens groups incoming, outgoing, and self-loop paths and follows neighbors', () => {
  ...
  assert.match(html, /Mosofin\.view\.reveal\(\[id\], \{ includeNeighbors: true, reason: 'relationship' \}\)/);
