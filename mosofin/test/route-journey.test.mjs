...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-route-journey-'));
...
test('motion, camera, layered Escape, mobile, print, and embed boundaries stay explicit', () => {
  assert.match(template, /Mosofin\.motionGovernor\.capable === true[\s\S]*?!Mosofin\.motionGovernor\.isPaused\(\)/);
  assert.match(template, /Mosofin\.motionGovernor\.claim\('route'/);
  assert.match(template, /reason: 'route-journey',[\s\S]*?maxScale: 1\.65,[\s\S]*?padding: 64,[\s\S]*?duration: 360/);
  assert.match(template, /Mosofin\.routeProbe\.pauseJourney\(\{ preserveElapsed: true, reason: reason \|\| 'manual' \}\)/);
  ...
  assert.match(template, /Mosofin\.routeProbe\.escape\(\{ restoreFocus: true \}\)/);
