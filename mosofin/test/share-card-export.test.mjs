...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-share-card-'));
...
test('Share Card uses contain-only canonical geometry with fixed safe areas', () => {
  ...
  assert.match(html, /MOSOFIN ·/);
  assert.doesNotMatch(svgBlock(html), /share-card|Share Card|MOSOFIN ·/);
});
