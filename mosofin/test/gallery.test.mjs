...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-gallery-'));
...
test('generated proof gallery matches its sources, receipts, and checked-in artifacts', () => {
  ...
  assert.match(output, /gallery 10 artifacts \/ 90 checks/);

  const manifestPath = path.join(generatedRoot, 'gallery', 'manifest.json');
  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  assert.equal(manifest.schemaVersion, 1);
  assert.equal(manifest.mosofinVersion, JSON.parse(fs.readFileSync(path.join(skillRoot, 'package.json'))).version);
  assert.equal(manifest.entryCount, 10);
  assert.equal(manifest.checkCount, 90);
  ...
  assert.deepEqual(
    ...
    { architecture: 3, workflow: 1, sequence: 1, dataflow: 3, lifecycle: 1, ledger: 1 },
  );
  assert.deepEqual(
  ...
  const workflow = manifest.entries.find((entry) => entry.id === 'close');
  assert.equal(workflow.view, 'reconcile-and-review');
  assert.equal(workflow.viewCount, 3);
  assert.deepEqual(workflow.viewIds, ['pull-sources', 'reconcile-and-review', 'breaks']);
  assert.equal(workflow.guidedPlayback, true);

  // The finance gallery never enables a fail-closed engineering profile silently.
  assert.ok(manifest.entries.every((entry) => entry.engineeringProfile === null));
  assert.ok(manifest.entries.every((entry) => /Northline Coffee|business runs/.test(entry.title)), 'every proof is a Northline business or finance artifact');
  ...
  assert.match(html, /data-src-base="gallery\/artifacts\/northline-close\.workflow\.html"/);
  assert.match(html, /northline-close\.workflow\.html\?present=1&amp;play=1#view=reconcile-and-review/);
  assert.match(html, /northline-payout-rec\.dataflow\.html\?present=1&amp;play=1#view=residual/);
  assert.match(html, /id="proof-money-map"/);
  assert.match(html, /id="proof-cash-runway"/);
  assert.match(html, /Play named chapter/);
  assert.match(html, /3 views · play/);
  assert.match(html, /See the diagrams\.<br><em>Then get started\.<\/em>/);
  assert.match(html, /id="gallery-slides"/);
  assert.match(html, /Four steps from add to a diagram you can forward\./);
  ...
  assert.match(html, /data-en="Share the map"/);
  assert.match(html, /Step 1 of 4/);
  assert.doesNotMatch(html, /Read versioned JSON IR|Run the typed renderer/);
  assert.match(html, /Six lenses\. Ten business stories\./);
  ...
  assert.doesNotMatch(html, /DEPLOYMENT OWNERSHIP · PASS/);
  assert.match(html, /\.nav-brand-wrapper \{[^}]*min-height: 44px;/);
  assert.match(html, /\.filter-button \{\s+min-height: 44px;/);
  assert.match(html, /\.card-link \{ min-height: 44px;/);
  assert.equal((html.match(/class="card-link create-link"/g) || []).length, 10);
  for (const type of ['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'ledger']) {
