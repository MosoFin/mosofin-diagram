...
test('release smokes the exact archive built for the release before freshness and upload', () => {
  ...
  assert.match(tagGate, /require\('\.\/mosofin\/package\.json'\)\.version/);
  assert.match(tagGate, /GITHUB_REF_NAME#v/);
  assert.match(build, /run: scripts\/build-zip\.sh \/tmp\/mosofin-built\.zip/);
  assert.match(smoke, /unzip -q \/tmp\/mosofin-built\.zip -d "\$package_root"/);
  assert.match(smoke, /node scripts\/package-smoke\.mjs "\$package_root\/mosofin"/);
  assert.doesNotMatch(smoke, /\bnpm\s+(?:ci|install)\b/);
  assert.match(freshness, /cmp -s \/tmp\/mosofin-built\.zip mosofin\.zip/);
  assert.match(upload, /files: mosofin\.zip/);
});
...
test('package smoke rejects every dependency or repository-only artifact', () => {
  ...
  for (const { relative, kind } of forbidden) {
    const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-package-gate-'));
    try {
      fs.mkdirSync(path.join(fixture, 'bin'), { recursive: true });
      fs.writeFileSync(path.join(fixture, 'bin', 'mosofin.mjs'), '');
...
canonicalZipTest('package smoke rejects every dependency metadata field in a built package', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-built-package-gate-'));
  try {
    const archive = path.join(fixture, 'mosofin.zip');
    ...
    const builtPackage = path.join(extracted, 'mosofin');
...
canonicalZipTest('archive build excludes untracked files and external symlinks from the live working tree', () => {
  ...
  const untracked = path.join(repoRoot, 'mosofin', `${marker}.txt`);
  const externalRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-package-external-'));
  const externalTarget = path.join(externalRoot, 'secret.txt');
  const externalLink = path.join(repoRoot, 'mosofin', `${marker}.link`);
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-package-negative-'));
  const archive = path.join(outputRoot, 'mosofin.zip');
...
canonicalZipTest('archive build rejects an unmerged index and preserves an existing archive', () => {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-package-unmerged-'));
  const scripts = path.join(fixture, 'scripts');
  const skill = path.join(fixture, 'mosofin');
  ...
  try {
    ...
    fs.writeFileSync(path.join(skill, 'package.json'), '{"name":"mosofin"}\n');
    ...
    const indexInfo = [
      `100644 ${base.stdout.trim()} 1\tmosofin/LICENSE`,
      `100644 ${ours.stdout.trim()} 2\tmosofin/LICENSE`,
      `100644 ${theirs.stdout.trim()} 3\tmosofin/LICENSE`,
      '',
    ].join('\n');
...
test('archive build rejects non-canonical Node versions before publishing output', {
  ...
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-package-node-version-'));
  try {
    const archive = path.join(outputRoot, 'mosofin.zip');
    ...
    assert.notEqual(build.status, 0, `${build.stdout}\n${build.stderr}`);
    assert.match(build.stderr, /canonical mosofin\.zip builds require Node 22/);
...
canonicalZipTest('archive build is byte-for-byte reproducible across caller time zones without system zip', () => {
  const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-package-reproducible-'));
