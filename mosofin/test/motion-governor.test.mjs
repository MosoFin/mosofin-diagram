...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-motion-governor-'));
...
test('all renderers inherit one viewer-only Live/Still Motion Governor', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Mosofin\.motionGovernor = \(function \(\)/, mode);
...
test('reader pause is persistent, explicit, and reduced-motion aware', () => {
  ...
  assert.match(html, /var STORAGE_KEY = 'mosofin-motion'/);
  ...
  assert.match(html, /Mosofin\.guidedViews\.isPlaying\(\)[\s\S]*?Mosofin\.guidedViews\.pause\(\)/);
...
test('strong semantic intent receives the single motion budget', () => {
  ...
  assert.match(html, /animation: mosofin-route-probe-flow 1\.1s[\s\S]*?1 both/);
  assert.doesNotMatch(html, /mosofin-route-probe-flow[^;]*infinite/);
});
