...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-animation-'));
...
test('signal-flow preset reaches the page, SVG, and motion export surface', () => {
  ...
  assert.match(html, /Mosofin\.view = \(function \(\)/);
...
test('editorial preset reaches every visual surface and all five typed renderers', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /data-preset-badge-editorial-plate="MOSOFIN \/ PLATE 04"/, mode);
...
test('all five renderers add one geometry-neutral semantic sigil per primary node', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    // Logo mode adds a second sigil per node as its no-logo fallback, inside a
    // .node-logoless group. This check is about the box view, so count only the
    // sigils the node box itself stamps.
    const sigils = (svg) => [...svg.matchAll(/<g aria-hidden="true" data-semantic-sigil="[^"]+"[\s\S]*?<\/g>/g)]
      .map((match) => match[0])
      .filter((_block, index, all) => {
        const at = svg.indexOf(all[index]);
        const before = svg.slice(Math.max(0, at - 260), at);
        return !before.includes('class="node-logoless"');
      });
...
for (const [mode, example] of Object.entries(CASES)) {
  test(`${mode}: trace animation annotates svg, edges, and nodes`, () => {
    ...
    assert.match(svg, /aria-labelledby="mosofin-diagram-title mosofin-diagram-description"/);
    assert.match(svg, /<title id="mosofin-diagram-title">[^<]+<\/title>/);
    assert.match(svg, /<desc id="mosofin-diagram-description">[^<]+<\/desc>/);
