...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-settled-flow-'));
...
test('all five renderers inherit one finite running-to-settled ambient contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /animation: mosofin-edge-flow [^;]+ 1;/, mode);
    assert.match(html, /animation: mosofin-node-pulse [^;]+ 1;/, mode);
    assert.doesNotMatch(html, /animation: mosofin-edge-flow [^;]+ infinite/, mode);
    assert.doesNotMatch(html, /animation: mosofin-node-pulse [^;]+ infinite/, mode);
  }
});
...
test('settled flow restores authored security and async dash semantics', () => {
  ...
  assert.match(template, /@keyframes mosofin-edge-flow\s*\{[\s\S]*?stroke-dasharray:\s*10 8/);
