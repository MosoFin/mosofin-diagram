...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-viewer-chrome-layout-'));
...
function render(mode, example) {
  ...
  execFileSync(process.execPath, [
    path.join(skillRoot, 'bin', 'mosofin.mjs'),
    'render',
    mode,
      ...
      return new Promise(function (resolve, reject) {
        ...
        function sample() {
          ...
          var current = [
            ...
            container ? getComputedStyle(container).getPropertyValue('--mosofin-nav-reserve') : ''
          ].join('|');
...
async function finalGeometry(browser, sessionId) {
  return evaluate(browser, sessionId, `(function () {
    ...
    return {
      reserve: parseFloat(getComputedStyle(container).getPropertyValue('--mosofin-nav-reserve')) || 0,
...
test('the public CLI gives all typed renderers one final Viewer contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(canonicalSvg(html), /nav-safe-rail|mosofin-nav-reserve|viewerChromeLayout/, mode);
    execFileSync(process.execPath, [path.join(skillRoot, 'bin', 'mosofin.mjs'), 'check', output]);
  }
});
