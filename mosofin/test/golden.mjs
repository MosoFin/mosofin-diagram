// Golden-file harness for the mosofin renderers. No test framework needed:
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-test-'));
...
for (const [mode, input, golden] of GOLDEN) {
  ...
  try {
    ...
    check(`${mode}: packaged ${golden}`, normalizeNewlines(fresh) === normalizeNewlines(packaged),
      `fresh render differs from mosofin/examples/${golden}; re-render the packaged examples and rebuild mosofin.zip`);
  } catch (err) {
...
check('template generator meta matches package.json version',
  template.includes(`<meta name="generator" content="mosofin ${pkg.version}">`),
