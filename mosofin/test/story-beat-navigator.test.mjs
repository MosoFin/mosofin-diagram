...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-story-beat-navigator-'));
...
test('one generation-owned scheduler resumes remaining dwell and finite exact-edge signals never loop', () => {
  ...
  assert.match(template, /animation: mosofin-story-flow 0\.78s linear 1 both/);
  assert.doesNotMatch(template, /mosofin-story-flow 0\.78s linear infinite/);
