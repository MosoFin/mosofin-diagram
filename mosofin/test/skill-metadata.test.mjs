...
test('main skill stays a bounded authoring router with progressive references', () => {
  ...
  for (const reference of [
    ...
    'references/finance-onboarding.md',
  ]) {
...
test('language behavior stays within the bounded locale contract', () => {
  ...
  assert.match(skill, /accepts `"en"` only/);
  assert.match(skill, /For any other requested language, omit `meta\.locale`/);
  assert.match(skill, /fixed Viewer UI and `<html lang>` remain English/);
  ...
  assert.match(authoringContract, /other than English/);
