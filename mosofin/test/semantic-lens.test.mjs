...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-semantic-lens-'));
...
test('all typed renderers inherit one viewer-only Semantic Lens', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Mosofin\.semanticLens = \(function \(\)/, mode);
...
test('Semantic Lens derives honest kind counts and compares at most two roles', () => {
  ...
  assert.match(html, /Choose up to two roles/);
...
test('Semantic Lens is shareable and yields cleanly to stronger reader intent', () => {
  ...
  assert.match(html, /Mosofin\.semanticLens\.clear\(\{ updateUrl: false/);
  assert.match(html, /Mosofin\.focus\.clear\(\{ updateUrl: false, preserveView: true \}\)/);
  assert.match(html, /Mosofin\.routeProbe\.clear\(\{ updateUrl: false, restoreFocus: false \}\)/);
  assert.match(html, /Mosofin\.guidedViews\.showAll\(\{ clearFocus: false, updateUrl: false \}\)/);
  assert.match(html, /if \(action === 'lens'\) return Mosofin\.semanticLens\.open\(\)/);
  assert.match(html, /e\.key === 'l' \|\| e\.key === 'L'/);
  assert.match(html, /e\.key === 'Escape' && Mosofin\.semanticLens\.isOpen\(\)/);
  assert.match(html, /e\.key === 'Escape' && Mosofin\.semanticLens\.active\(\)/);
});
