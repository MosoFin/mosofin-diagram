...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-presentation-'));
...
test('all typed renderers ship the same presentation stage contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /Mosofin\.presentation = \(function \(\)/, mode);
...
test('presentation keyboard behavior exits in layers and remains accessible', () => {
  ...
  assert.match(html, /Mosofin\.presentation\.toggle\(\)/);
  assert.match(html, /e\.key === 'Escape' && Mosofin\.focus\.active\(\)/);
  assert.match(html, /e\.key === 'Escape' && Mosofin\.presentation\.active\(\)/);
