...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-webm-artifact-'));
const externalReachSource = process.env.MOSOFIN_REACH_CARD_SOURCE
  ? path.resolve(process.env.MOSOFIN_REACH_CARD_SOURCE)
  : '';
const externalReachOutput = process.env.MOSOFIN_REACH_CARD_OUTPUT
  ? path.resolve(process.env.MOSOFIN_REACH_CARD_OUTPUT)
  : '';
assert.equal(Boolean(externalReachSource), Boolean(externalReachOutput), 'MOSOFIN_REACH_CARD_SOURCE and MOSOFIN_REACH_CARD_OUTPUT must be set together');
...
const ffmpeg = executable([process.env.MOSOFIN_FFMPEG, 'ffmpeg']);

assert.ok(chrome, 'Chrome/Chromium is required for the WebM artifact smoke test (or set MOSOFIN_CHROME)');
assert.ok(ffmpeg, 'ffmpeg is required for the WebM artifact smoke test (or set MOSOFIN_FFMPEG)');
...
const specialSourceLabel = 'ingress <script>window.__routeLabelExecuted = true</script> hangul 한글 🚀 with an intentionally very long endpoint label';
const specialTargetLabel = 'egress service ✅ emoji + hangul + a second deliberately long endpoint label for header fitting';
...
const specialRouteSource = {
  ...
  diagram_type: 'architecture',
  meta: {
    title: 'Wide-glyph Route Share Card 🚀 with a deliberately long original diagram title that must fit safely',
...
try {
  ...
  async function verifyResolvedLegendContract(outputs) {
    async function inspectKinds(file, expectedKinds, theme) {
      ...
      await navigateReady(url, '!!(window.Mosofin && Mosofin.semanticLens)', `legend ${theme}`);
    ...
    for (const theme of ['dark', 'light']) {
      ...
      assert.deepEqual(dataflow.aria, ['Inspect account, 1 node', null]);
    ...
    await navigateReady(outputs.dataflow, '!!(window.Mosofin && Mosofin.semanticLens && Mosofin.exportMenu)', 'Dataflow database legend runtime');
    const databaseRuntime = await evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      URL.createObjectURL = function (blob) {
        captured = blob.text();
        return 'blob:mosofin-dataflow-legend-smoke';
      };
      HTMLAnchorElement.prototype.click = function () {};
      try {
        await Mosofin.exportMenu.run('svg');
        var exportedText = await captured;
        var exported = new DOMParser().parseFromString(exportedText, 'image/svg+xml').documentElement;
        return {
          selected: Mosofin.semanticLens.active(),
          lensOpen: Mosofin.semanticLens.isOpen(),
    ...
    await navigateReady(outputs.custom, '!!(window.Mosofin && Mosofin.semanticLens && Mosofin.exportMenu)', 'custom legend runtime');
    const runtime = await evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var selected = Mosofin.semanticLens.active();
      var guidedActivated = Mosofin.guidedViews.activate('main', { updateUrl: false });
      var guidedActive = Mosofin.guidedViews.active();
      var visualMatrix = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!Mosofin.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
      ...
      URL.createObjectURL = function (blob) {
        captured = blob.text();
        return 'blob:mosofin-legend-smoke';
      };
      HTMLAnchorElement.prototype.click = function () {};
      try {
        await Mosofin.exportMenu.run('svg');
    ...
    assert.deepEqual(runtime.labels, ['Inspect Reader <UI> & ops, 1 node', 'Inspect Ledger, 1 node']);
    ...
    for (const entry of runtime.visualMatrix) {
      ...
      assert.deepEqual(entry.labels, ['Reader <UI> & ops', 'Ledger', 'Future integration']);
    }
    ...
    assert.deepEqual(runtime.exportedLabels, ['Reader <UI> & ops', 'Ledger', 'Future integration']);
    ...
    await navigateReady(embedUrl, '!!(window.Mosofin && Mosofin.semanticLens)', 'embedded legend');
    ...
    await navigateReady(outputs.hidden, '!!(window.Mosofin && Mosofin.semanticLens)', 'hidden legend');
  ...
  async function verifySemanticPassportDismissal(file) {
    await navigateReady(file, '!!(window.Mosofin && Mosofin.focus && document.querySelector("#btn-focus-clear"))', 'Semantic Passport dismissal');
    const result = await evaluate(cdp, sessionId, `(() => {
      ...
      var origin = svg.querySelector('[data-node-id="shopify"]');
      var neighbor = svg.querySelector('[data-node-id]:not([data-node-id="shopify"])');
      if (!origin || !neighbor) return { ok: false, error: 'missing smoke-test nodes' };
      function state() {
        return { hidden: chip.hidden, active: Mosofin.focus.active() };
      }

      Mosofin.focus.set('shopify', { toggle: false, updateUrl: false });
    ...
    assert.equal(result.restoredFocus, 'shopify');
    assert.deepEqual(result.afterInside, { hidden: false, active: 'shopify' });
  ...
  async function verifyArchitectureDeltaNavigator(file) {
    ...
    const exportProof = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var explorers = frames.map(function (frame) {
        ...
        return Boolean(child && child.Mosofin && child.Mosofin.focus && child.Mosofin.routeProbe && child.document.querySelector('#btn-node-finder') && child.document.querySelector('#guided-view-play'));
      });
      var svgA = Mosofin.deltaExport.canonicalSvg();
      document.querySelector('#theme').click();
      document.querySelector('#preset').click();
      document.querySelector('.change-row').click();
      var svgB = Mosofin.deltaExport.canonicalSvg();
      var parsed = new DOMParser().parseFromString(svgB, 'image/svg+xml');
      var exportStyle = parsed.querySelector('style')?.textContent || '';
      var blob = await Mosofin.deltaExport.shareCard();
  ...
  async function captureShareCard(file, label) {
    await navigateReady(file, '!!(window.Mosofin && Mosofin.exportMenu && Mosofin.exportMenu.shareCard)', label);
    const sharePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        var blob = await Mosofin.exportMenu.shareCard();
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
  ...
  async function captureCopiedShareCard(file, label) {
    await navigateReady(file, '!!(window.Mosofin && Mosofin.exportMenu && Mosofin.exportMenu.copyShareCard)', label);
        ...
        Object.defineProperty(navigator, 'clipboard', {
          ...
          value: {
            write: async function (items) {
              window.__mosofinCopiedShareCard = await Promise.resolve(items[0].items['image/png']);
            }
          }
        });
        window.alert = function (message) { window.__mosofinCopyAlert = message; };
        await Mosofin.exportMenu.copyShareCard();
        var blob = window.__mosofinCopiedShareCard;
        if (!blob) throw new Error(window.__mosofinCopyAlert || 'clipboard received no blob');
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
        for (var offset = 0; offset < bytes.length; offset += 32768) {
          binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
  ...
  async function captureRouteShareCard(file, label, sourceId, targetId, options = {}) {
    await navigateReady(file, '!!(window.Mosofin && Mosofin.routeProbe && Mosofin.exportMenu && Mosofin.exportMenu.downloadRouteShareCard)', label);
    const routePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        window.alert = function (message) { window.__mosofinRouteAlert = message; };
        Mosofin.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
        if (!Mosofin.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
          throw new Error('route did not resolve');
        }
        var snapshot = Mosofin.routeProbe.exportSnapshot();
        if (!snapshot) throw new Error('resolved route exposed no export snapshot');
        Mosofin.exportMenu.syncRouteShare();
        ...
        var duplicateGeometryRejected = Mosofin.routeProbe.exportSnapshot() === null;
        var duplicateExportError = '';
        try { await Mosofin.exportMenu.shareCard({ variant: 'route' }); }
        ...
        var emptyGeometryRejected = Mosofin.routeProbe.exportSnapshot() === null;
        ...
        try {
          blob = await Mosofin.exportMenu.downloadRouteShareCard();
          routeReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
          ...
          if (${JSON.stringify(options.journeyInvariance === true)}) {
            async function captureJourneySource(action) {
              ...
              await Mosofin.exportMenu.shareCard({ variant: 'route' });
              routeFingerprints.push(fingerprint(await captured[captureIndex]));
            }
            await captureJourneySource(function () { Mosofin.routeProbe.selectJourneyIndex(0); });
            await captureJourneySource(function () { Mosofin.routeProbe.selectJourneyIndex(Math.floor(snapshot.nodeIds.length / 2)); });
            await captureJourneySource(function () { Mosofin.routeProbe.selectJourneyIndex(snapshot.nodeIds.length - 1); });
            Mosofin.routeProbe.showOverview({ reveal: false });
            var started = Mosofin.routeProbe.playJourney();
            ...
            Mosofin.routeProbe.pauseJourney({ preserveElapsed: false });
            await captureJourneySource(function () {});
          ...
          await Mosofin.exportMenu.run('png');
          var pngReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            canonical: document.documentElement.getAttribute('data-last-export-canonical')
          ...
          var asyncClearPromise = Mosofin.exportMenu.shareCard({ variant: 'route' });
          Mosofin.routeProbe.clear({ updateUrl: false, preserveView: true });
          ...
          await Mosofin.exportMenu.downloadRouteShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
          ...
          return {
            ...
            staleSnapshot: Mosofin.routeProbe.exportSnapshot(),
  ...
  async function verifyDynamicReducedMotionRoute(file, label, sourceId, targetId) {
    ...
    await navigateReady(file, '!!(window.Mosofin && Mosofin.routeProbe && Mosofin.exportMenu && Mosofin.motionGovernor)', label);
    ...
    const setup = await evaluate(cdp, sessionId, `(function () {
      Mosofin.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!Mosofin.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) return { resolved: false };
      Mosofin.routeProbe.showOverview({ reveal: false });
      return {
        resolved: true,
        started: Mosofin.routeProbe.playJourney(),
        playing: Mosofin.routeProbe.isJourneyPlaying(),
  ...
  async function captureRouteVisualMatrix(file, label, sourceId, targetId) {
    await navigateReady(file, '!!(window.Mosofin && Mosofin.preset && Mosofin.routeProbe && Mosofin.exportMenu)', label);
    const matrix = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      Mosofin.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!Mosofin.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
        throw new Error('route did not resolve');
      }
      var identity = JSON.stringify(Mosofin.routeProbe.exportSnapshot());
      var results = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!Mosofin.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
        for (var theme of ['dark', 'light']) {
          document.documentElement.setAttribute('data-theme', theme);
          var blob = await Mosofin.exportMenu.shareCard({ variant: 'route' });
          var bytes = new Uint8Array(await blob.arrayBuffer());
          var hash = 2166136261;
          ...
          results.push({
            ...
            identity: JSON.stringify(Mosofin.routeProbe.exportSnapshot())
          });
        }
      }
  ...
  async function captureReachShareCard(file, label, originId, direction, options = {}) {
    await navigateReady(file, '!!(window.Mosofin && Mosofin.focus && Mosofin.focus.reachabilitySnapshot && Mosofin.exportMenu && Mosofin.exportMenu.downloadReachShareCard)', label);
    const reachPayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        window.alert = function (message) { window.__mosofinReachAlert = String(message); };
        if (!Mosofin.focus.set(${JSON.stringify(originId)}, { toggle: false, updateUrl: false })) {
          throw new Error('focus origin did not resolve');
        }
        if (!Mosofin.focus.reach(${JSON.stringify(direction)}, { toggle: false, updateUrl: false, reveal: false })) {
          throw new Error('authored reach did not resolve');
        }
        var snapshot = Mosofin.focus.reachabilitySnapshot();
        if (!snapshot) throw new Error('active authored reach exposed no export snapshot');
        Mosofin.exportMenu.syncReachShare();
        ...
        svg.appendChild(duplicateCarrier);
        var duplicateGeometryRejected = Mosofin.focus.reachabilitySnapshot() === null;
        var duplicateExportError = '';
        try { await Mosofin.exportMenu.shareCard({ variant: 'reach' }); }
        catch (error) { duplicateExportError = String(error && error.message || error); }
        duplicateCarrier.remove();
        snapshot = Mosofin.focus.reachabilitySnapshot();
        ...
        try {
          var blob = await Mosofin.exportMenu.shareCard({ variant: 'reach' });
          var reachSvgText = captured[0] ? await captured[0] : '';
          var downloadedBlob = await Mosofin.exportMenu.downloadReachShareCard();
          var reachReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            width: document.documentElement.getAttribute('data-last-export-width'),
          ...
          if (${JSON.stringify(options.matrix === true)}) {
            ...
            for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
              ...
              for (var theme of ['dark', 'light']) {
                document.documentElement.setAttribute('data-theme', theme);
                var matrixBlob = await Mosofin.exportMenu.shareCard({ variant: 'reach' });
                ...
                matrix.push({
                  ...
                  identity: JSON.stringify(Mosofin.focus.reachabilitySnapshot())
                });
              }
            }
          ...
          Mosofin.focus.clearReach({ updateUrl: false });
          Mosofin.exportMenu.syncReachShare();
          ...
          await Mosofin.exportMenu.downloadReachShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            errorFormat: document.documentElement.getAttribute('data-last-export-error-format'),
          ...
          var canonicalIndex = captured.length;
          var canonicalBlob = await Mosofin.exportMenu.shareCard();
          var canonicalSvgText = captured[canonicalIndex] ? await captured[canonicalIndex] : '';
          var canonicalSvg = parser.parseFromString(canonicalSvgText, 'image/svg+xml').documentElement;
          ...
          return {
            ...
            hiddenAfterClear: hiddenAfterClear,
            staleSnapshot: Mosofin.focus.reachabilitySnapshot(),
            staleError: staleError,
            failedReceipt: failedReceipt,
            canonicalSize: canonicalBlob.size,
  ...
  await verifySemanticPassportDismissal(path.resolve(skillRoot, '../docs/gallery/artifacts/northline-money-map.architecture.html'));
  ...
  if (externalReachSource) {
    ...
    await captureReachShareCard(
      externalReachSource,
      'external-reach',
      process.env.MOSOFIN_REACH_CARD_ORIGIN || 'router',
      process.env.MOSOFIN_REACH_CARD_DIRECTION || 'downstream',
  ...
  await navigateReady(output, '!!(window.Mosofin && Mosofin.motion && Mosofin.motion.canRecord())', 'motion artifact');

  const payload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
    try {
      var blob = await Mosofin.motion.recordWebm({ duration: 1400, fps: 12 });
      var bytes = new Uint8Array(await blob.arrayBuffer());
      var binary = '';
      for (var offset = 0; offset < bytes.length; offset += 32768) {
        binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
