...
function usage() {
  return `Usage:
  mosofin render <type> <input.json> [output.html] [--quality standard|showcase] [--repo-root path]
  mosofin compare architecture <base.json> <head.json> [output.html] [--receipt path] [--json] [--quality standard|showcase] [--repo-root path]
  mosofin deliver <type> <input.json> [output.html] [--json] [--open] [--quality standard|showcase] [--repo-root path]
  mosofin preview <type> <input.json> [output.html] [--no-open] [--quality standard|showcase] [--repo-root path]
  mosofin validate <type> <input.json> [--json] [--layout-json] [--quality standard|showcase] [--repo-root path]
...
function rendererEnv(quality, repoRoot, diagnosticJson = false) {
  return {
    ...(quality ? { MOSOFIN_QUALITY_PROFILE: quality } : {}),
    ...(repoRoot ? { MOSOFIN_REPO_ROOT: repoRoot } : {}),
    ...(diagnosticJson ? { MOSOFIN_DIAGNOSTIC_FORMAT: 'json' } : {}),
  };
}
...
async function commandCompare(args) {
  ...
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'prepare', error: 'Architecture compare runtime is unavailable.', code: 'delta/runtime-missing', details: { reason: error.message, supportedFixes: ['install the complete Mosofin skill package'] } });
    return;
  }
  const {
  ...
  try {
    ...
    const outputDiagnostic = error.mosofinDiagnostics?.[0];
  ...
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.mosofin-compare-'));
  } catch (error) {
...
function sourceEvidenceFromArtifact(artifact) {
  ...
  const match = html.match(/<script id="mosofin-source-evidence-data" type="application\/json">([\s\S]*?)<\/script>/);
...
async function commandDeliver(args) {
  ...
  try {
    ...
    reportDeliveryFailure({
      ...
      error: error.message,
      diagnostics: error.mosofinDiagnostics || [diagnostic({
  ...
  let stagingDirectory;
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.mosofin-delivery-'));
  } catch (error) {
...
async function commandDoctor() {
  ...
  const authoringReferences = [
    ...
    path.join(skillRoot, 'references', 'finance-onboarding.md'),
  ];
  ...
  console.error(`\nMosofin is not ready: ${problems.join('; ')}.`);
  process.exitCode = 1;
}
...
async function commandBrands(args) {
  ...
  if (positional[0] === 'capture') {
    if (positional.length !== 2) fail('Usage: mosofin brands capture <url> [--json]');
  ...
  if (json) {
    console.log(JSON.stringify({
      ...
      count: marks.length,
      marks,
      fallback: 'Run "mosofin brands capture <url> --json", then use the returned digest-pinned brand value.',
    }, null, 2));
    return;
  }
  if (!marks.length) {
    console.log(`No built-in brand matched "${query}". Run "mosofin brands capture <url> --json", then use the returned digest-pinned brand value.`);
    return;
  }
...
function commandDemo(args) {
  ...
  const output = path.join(outputDirectory, 'mosofin-demo.html');
  ...
  console.log('  mosofin render architecture <input.json> <output.html>');
}
...
function commandValidate(args) {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mosofin-validate-'));
