...
const cliPath = path.join(here, 'mosofin.mjs');
const loopbackHost = '127.0.0.1';
const defaultDebounceMs = 400;
const defaultPollMs = 800;
const defaultStopGraceMs = 3000;
const defaultStopKillMs = 750;
const diagramTypes = new Set(['architecture', 'workflow', 'sequence', 'dataflow', 'lifecycle', 'ledger']);
...
<head>
  ...
  <title>Mosofin Live Preview</title>
  <style>
...
<body data-state="checking" data-has-artifact="false">
  <header>
    <span class="brand">Mosofin Preview</span>
  ...
  <main>
    ...
    <iframe id="artifact" title="Verified Mosofin diagram"></iframe>
  </main>
  <script>
    (function () {
      'use strict';
...
export async function startPreview(options) {
  ...
  const stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.mosofin-preview-'));
    ...
    state.failure = {
      ...
      message: redactDiagnostic(
        ...
        [
          ...
          [path.resolve(here, '..'), '<mosofin-skill>'],
