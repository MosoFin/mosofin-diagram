...
| Show me the month moving through the books, entry by entry | `ledger` | at most 12 accounts, authored credit-to-debit flows, the GL rows as events, split entries unmapped, tie-outs only for supplied figures; `meta.view: "city"` for the isometric district (same data); optional `ledger.scenarios[]` for labelled what-if overlays (hollow tokens; never mixed into proof/tie-outs) |
